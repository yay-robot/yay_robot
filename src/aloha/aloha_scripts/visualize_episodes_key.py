...
def load_hdf5(dataset_dir, dataset_name):
    ...
    with h5py.File(dataset_path, 'r') as root:
        ...
        option = root['/observations/option'][()]
        action = root['/action'][()]
        image_dict = dict()
...
def main(args):
    ...
    save_videos(image_dict, DT, option, video_path=os.path.join(dataset_dir, dataset_name + '_video_key.mp4'))
...
def save_videos(video, dt, options, video_path=None):
    ...
    if isinstance(video, list):
        ...
        for ts, image_dict in enumerate(video):
            ...
            for cam_name in cam_names:
                ...
                cv2.putText(image_copy, option_labels[int(options[ts])], (10, 50), font, font_scale, font_color, font_thickness, lineType=cv2.LINE_AA)
                image = image_copy
        ...
        for t in range(n_frames):
            ...
            # Overlay option label on the image
            image_copy = image.copy()
            cv2.putText(image_copy, option_labels[int(options[t])], (10, 50), font, font_scale, font_color, font_thickness, lineType=cv2.LINE_AA)
            image = image_copy
