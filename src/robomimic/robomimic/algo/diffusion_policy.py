...
def replace_bn_with_gn(root_module: nn.Module, features_per_group: int=16) -> nn.Module:
    ...
    """
    def group_norm_factory(x: nn.Module) -> nn.Module:
        num_groups = gcd(x.num_features, features_per_group)
        return nn.GroupNorm(num_groups=num_groups, num_channels=x.num_features)

    replace_submodules(
        root_module=root_module,
        predicate=lambda x: isinstance(x, nn.BatchNorm2d),
        func=group_norm_factory
    )
    return root_module
...
class ConditionalUnet1D(nn.Module):
    def __init__(self, 
        ...
        # print("number of parameters of ConditionalUnet1D: {:e}".format(
