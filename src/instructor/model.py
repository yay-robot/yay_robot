...
class Instructor(nn.Module):
    def __init__(self, device, history_len, output_size=768, hidden_size=512, num_heads=8, num_layers=6, candidate_embeddings=None, candidate_texts=None, command_to_index=None, num_cameras=4):
        super().__init__()
        ...
        self.transformer = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(d_model=self.clip_model.visual.output_dim, nhead=num_heads, dim_feedforward=hidden_size),
    ...
    def forward(self, images):
        ...
        images_transformed = clip_transform(images_reshaped) # CLIP model expects images to be normalized and resized to 224*224
        ...
        image_features_reshaped = image_features.reshape(batch_size, timesteps * num_cameras, -1).to(torch.float32)

        # Add positional encoding
        image_features_reshaped += self.positional_encoding[:timesteps * num_cameras, :].to(image_features_reshaped.device)
        
        # Pass the concatenated features through the Transformer
        transformer_out = self.transformer(image_features_reshaped.transpose(0, 1)).transpose(0, 1)
        ...
        if ONE_HOT:
            ...
            logits = self.compute_similarities(command_pred) / self.temperature.clamp(min=1e-8)
...
######################    
    def compute_similarities(self, embeddings):
        ...
        cosine_similarities = (embeddings @ self.candidate_embeddings.T) / (
            embeddings.norm(dim=-1, keepdim=True) * self.candidate_embeddings.norm(dim=-1, keepdim=True).T)
    ...
    def decode_logits(self, logits, temperature):
        ...
        probs = logits if ONE_HOT else torch.nn.functional.softmax(logits / temperature, dim=-1) 
    ...
    def get_random_from_top_k(self, embeddings, k=3):
        similarities = (embeddings @ self.candidate_embeddings.T) / (embeddings.norm(dim=-1, keepdim=True) * self.candidate_embeddings.norm(dim=-1, keepdim=True).T)
        top_k_indices = similarities.topk(k, dim=-1)[1]
        
        # Randomly select one from the top-k for each row
        selected_indices = [random.choice(indices_row) for indices_row in top_k_indices.cpu().numpy()]
    ...
    def sample_with_temperature(self, embeddings, temperature=1.0):
        ...
        sampled_indices = torch.multinomial(probs, 1).squeeze()  # Squeezing to potentially remove singleton dimensions
...
if __name__ == "__main__":
    ...
    # Load the dataloader
    train_dataloader, val_dataloader, _ = load_data(dataset_dir, num_episodes, camera_names, batch_size_train, batch_size_val)
