...
def signal_handler(sig, frame):
    global EXIT_FLAG
    print(
        "Keyboard interrupt detected. Waiting for data processing tasks to complete..."
    )
    EXIT_FLAG = True
...
def data_collection(
    ...
    if AUDIO:
        ...
        audio_recording = audio_recording[
            :recorded_frames
        ]  # only save the portion that contains recorded audio

        audio_recording_int16 = (audio_recording * (2**15 - 1)).astype(np.int16)
        wavio.write(
            os.path.join(dataset_dir, dataset_name + ".wav"),
            audio_recording_int16,
            audio_sampling_rate,
            sampwidth=2,
        )
...
def data_processing(
    max_timesteps, camera_names, dataset_path, episode_queue, collect_done_event
):
    """
    For each timestep:
    observations
    ...
    while not collect_done_event.is_set() or not episode_queue.empty():
        try:
            ts, action = episode_queue.get(
                timeout=2
            )  # This will block until there's data in the queue
        except queue.Empty:
            continue
...
def main(args):
    ...
    for _ in range(num_episodes):
        ...
        episode_queue, data_collection_thread = capture_one_episode(
            DT, max_timesteps, camera_names, dataset_dir, dataset_name, overwrite=True
        )
        ...
        thread = threading.Thread(
            target=threaded_data_processing,
            args=(
                semaphore,
                max_timesteps,
                camera_names,
