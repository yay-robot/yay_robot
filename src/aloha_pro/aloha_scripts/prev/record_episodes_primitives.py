...
def on_press(key):
    global primitive_num, should_reset, should_end
    try:
        primitive_num = int(key.char)
        print(f'\nCurrent Primitive Number : {primitive_num}')
...
def capture_one_episode(dt, max_timesteps, camera_names, dataset_dir, dataset_name, overwrite, num_episodes):
    ...
    for t in tqdm(range(max_timesteps)):
        ...
        if should_reset:
            should_reset = False
            primitive_num = 0
            capture_one_episode(dt, max_timesteps, camera_names, dataset_dir, dataset_name, overwrite)
            return
        if should_end:
            should_end = False
            break
        ...
        t1 = time.time() #
        ts = env.step(action)
        ts.observation['primitive_num'] = primitive_num
        t2 = time.time() #
        timesteps.append(ts)
        actions.append(action)
        primitive_history.append(primitive_num)
    ...
    # if not (set([0,1,2]) == set(primitive_history)):
    #     print("Primitive History incorrect, Restarting")
    #     should_reset = False
    #     primitive_num = 0
    #     capture_one_episode(dt, max_timesteps, camera_names, dataset_dir, dataset_name, overwrite)
    #     return
    ...
    while actions:
        ...
        primitive = primitive_history.pop(0)
        ts = timesteps.pop(0)
        data_dict['/observations/qpos'].append(ts.observation['qpos'])
        data_dict['/observations/qvel'].append(ts.observation['qvel'])
        data_dict['/observations/effort'].append(ts.observation['effort'])
        # data_dict['/observations/primitive_num'].append(primitive)
        data_dict['/action'].append(action)
        for cam_name in camera_names:
            data_dict[f'/observations/images/{cam_name}'].append(ts.observation['images'][cam_name])
            # data_dict[f'/observations/images/{cam_name}_depth'].append(ts.observation['images'][f'{cam_name}_depth'])
    data_dict['/observations/primitive_num'] = np.expand_dims(np.array(data_dict['/observations/primitive_num']), axis=1)
    ...
    if save_input[-1] == '4':
        with h5py.File(dataset_path + '.hdf5', 'w') as root:
            ...
            for cam_name in camera_names:
                _ = image.create_dataset(cam_name, (actual_length, 480, 640, 3), dtype='uint8',
                                        chunks=(1, 480, 640, 3), )
                # compression='gzip',compression_opts=2,)
                # compression=32001, compression_opts=(0, 0, 0, 0, 9, 1, 1), shuffle=False)
            _ = obs.create_dataset('primitive_num', (actual_length, 1))
            _ = obs.create_dataset('qpos', (actual_length, 14))
            _ = obs.create_dataset('qvel', (actual_length, 14))
            _ = obs.create_dataset('effort', (actual_length, 14))
            _ = root.create_dataset('action', (actual_length, 14))
            # e()
...
def main(args):
    ...
    print(dataset_name + '\n')
    capture_one_episode(DT, max_timesteps, camera_names, dataset_dir, dataset_name, overwrite, num_episodes)
    # while True:
