...
class DiffusionPolicy(nn.Module):
    def __init__(self, args_override):
        ...
        if use_language:
            ...
            self.lang_embed_proj = nn.Linear(768, self.feature_dimension) # TODO: might change
            in_shape = [1536, 10, 10]
        else:
            in_shape = [512, 15, 20]
        ...
        for _ in self.camera_names:
            ...
            pools.append(SpatialSoftmax(**{'input_shape': in_shape, 'num_kp': self.num_kp, 'temperature': 1.0, 'learnable_temperature': False, 'noise_std': 0.0}))
    ...
    def __call__(self, qpos, image, actions=None, is_pad=None, command_embedding=None):
        ...
        if actions is not None: # training time
            ...
            if command_embedding is None:
                ...
                obs_cond = torch.cat(all_features + [qpos] + [command_embedding_proj], dim=1)
