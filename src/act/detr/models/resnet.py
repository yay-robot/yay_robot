...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None, groups=1, base_width=64, dilation=1, norm_layer=None, use_film=False, language_embed_size=768):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None, groups=1, base_width=64, dilation=1, norm_layer=None, use_film=False, language_embed_size=768):
        ...
        self.use_film = use_film
        if self.use_film:
            self.film = FiLMBlock(language_embed_size=language_embed_size, num_channels=planes * self.expansion)
...
class CustomResNet(ResNet):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, use_film=False):
        if dilate:
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer, use_film))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, use_film=use_film))
    ...
    def forward(self, x, language_embed=None):
        ...
        x = self.layer1(x, language_embed) if isinstance(self.layer1[0], (BasicBlock, Bottleneck)) and self.layer1[0].use_film else self.layer1(x)
        x = self.layer2(x, language_embed) if isinstance(self.layer2[0], (BasicBlock, Bottleneck)) and self.layer2[0].use_film else self.layer2(x)
        x = self.layer3(x, language_embed) if isinstance(self.layer3[0], (BasicBlock, Bottleneck)) and self.layer3[0].use_film else self.layer3(x)
        x = self.layer4(x, language_embed) if isinstance(self.layer4[0], (BasicBlock, Bottleneck)) and self.layer4[0].use_film else self.layer4(x)
...
def filmed_forward(self, x, language_embed):
    ...
    film = FiLMBlock(language_embed_size=language_embed.size(1), num_channels=out.size(1))
    ...
    if use_film:
        for n, m in model.named_modules():
            if isinstance(m, (BasicBlock, Bottleneck)):
                m.forward = lambda x, language_embed: filmed_forward(m, x, language_embed)
    if weights:
