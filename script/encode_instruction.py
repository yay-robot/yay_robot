...
if __name__ == "__main__":
    ...
    if args.from_count:
        ...
        generate_embeddings_file(candidate_texts, encoder, tokenizer, model, output_file)
    else:
        # List of files to process
        files_to_process = [f for f in os.listdir(dataset_dir) if "episode_" in f and f.endswith(".json") and "_encoded" not in f]
        ...
        for file_name in tqdm(files_to_process, desc="Processing files"):
            process_file(os.path.join(dataset_dir, file_name), encoder, tokenizer, model)
