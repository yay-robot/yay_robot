...
def process_episode(dataset_dir, episode_name):
    ...
    if len(new_instruction_positions) != len(raw_instructions):
        print(f"Warning: {episode_name}\tdifference is {abs(len(new_instruction_positions) - len(raw_instructions))}\t\tnumber of instructions ({len(raw_instructions)})\tnumber of -1 values ({len(new_instruction_positions)})")

    instruction_data = []

    for idx in range(len(new_instruction_positions)):
        start = new_instruction_positions[idx]
        end = new_instruction_positions[idx + 1] - 1 if idx + 1 < len(new_instruction_positions) else len(option_values) - 1
        
        next_val = option_values[start + 1] if start + 1 < len(option_values) else None
        instruction_type = "action" if next_val == 0 else "instruction" if next_val in [1, 3] else "correction"
