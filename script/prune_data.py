...
def process_hdf5_data(file_path, output_dir):
    with h5py.File(file_path, "r") as f:
        ...
        while idx < len(options_data):
            ...
            new_data["qpos"].append(
                qpos_data[idx - 1]
            )  # Repeated data for transition marker
        ...
        with h5py.File(output_path, "w") as out_f:
            ...
            out_f.create_dataset(
                "/observations/effort", data=np.array(new_data["effort"])
            )
            out_f.create_dataset(
                "/observations/option", data=np.array(new_data["option"])
            )
            ...
            if compress_len_data is not None:
                ...
                while idx < len(options_data):
                    ...
                    current_option = int(options_data[idx])
                    while (
                        idx < len(options_data) and options_data[idx] == current_option
                    ):
                        idx += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process HDF5 files to remove frozen timesteps and relabel options."
    )
    parser.add_argument(
        "--data_dir", type=str, default=".", help="Directory containing the HDF5 files."
    )
    parser.add_argument(
        "--start_idx", type=int, required=True, help="Starting episode index."
    )
    parser.add_argument(
        "--end_idx", type=int, required=True, help="Ending episode index."
    )
    ...
    hdf5_files = [
        os.path.join(args.data_dir, f"episode_{i}.hdf5")
