...
directory = "/scr/lucyshi/dataset/aloha_bag_3_objects_johnny_v1"
...
def rename_files(directory):
    # Find all episodes
    episodes = sorted(set(int(filename.split('_')[1].split('.')[0]) for filename in os.listdir(directory) if filename.startswith('episode_')))
    ...
    for old_episode, new_episode in episode_map.items():
        for file_type in file_types:
            ...
            if os.path.exists(old_filename) and old_episode != new_episode:
                # print(f"Renaming {old_episode}{file_type} to {new_episode}{file_type}")
